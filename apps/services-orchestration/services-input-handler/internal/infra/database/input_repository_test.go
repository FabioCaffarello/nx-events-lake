...
func (suite *InputRepositorySuite) TestSaveWhenIsANewInput() {
	input, err := entity.NewInput(map[string]interface{}{"test": "test"}, suite.Source, suite.Collection, suite.Context)
	suite.NoError(err)
	err = suite.repo.Save(input, suite.Collection)
	suite.NoError(err)
}
...
func (suite *InputRepositorySuite) TestFindAllByService() {
	...
	input1, err := entity.NewInput(data1, suite.Source, suite.Collection, suite.Context)
	suite.NoError(err)
	err = suite.repo.Save(input1, suite.Collection)
	suite.NoError(err)

	data2 := map[string]interface{}{
		"test": "test2",
	}
	input2, err := entity.NewInput(data2, suite.Source, suite.Collection, suite.Context)
	suite.NoError(err)
	err = suite.repo.Save(input2, suite.Collection)
	suite.NoError(err)
...
func (suite *InputRepositorySuite) TestSaveAndUpdateInput_NewInput() {
	input, err := entity.NewInput(map[string]interface{}{"test": "test"}, suite.Source, suite.Collection, suite.Context)
	suite.NoError(err)
...
func (suite *InputRepositorySuite) TestSaveAndUpdateInput_ExistingInput() {
	input, err := entity.NewInput(map[string]interface{}{"test": "test"}, suite.Source, suite.Collection, suite.Context)
	suite.NoError(err)
