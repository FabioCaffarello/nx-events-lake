...
func (suite *SchemaRepositorySuite) TestSaveWhenSchemaDoesNotExist() {
	jsonSchema := map[string]interface{}{
		"test": "test",
	}
	schema, err := entity.NewSchema("test", "test", "test", "test", jsonSchema)
	suite.NoError(err)
	err = suite.repo.Save(schema)
	suite.NoError(err)
}
...
func (suite *SchemaRepositorySuite) TestFindAll() {
	...
	schema1, err := entity.NewSchema("test1", "test", "test", "test1", jsonSchema1)
	suite.NoError(err)
	schema2, err := entity.NewSchema("test2", "test", "test", "test2", jsonSchema2)
	suite.NoError(err)

	err = suite.repo.Save(schema1)
	suite.NoError(err)
	err = suite.repo.Save(schema2)
	suite.NoError(err)
...
func (suite *SchemaRepositorySuite) TestSaveWhenSchemaExist() {
	jsonSchema := map[string]interface{}{
	...
	schema, err = entity.NewSchema("test", "test", "test", "test", newJsonSchema)
	suite.NoError(err)
